# A era 10x chegou. Dez vezes mais o quê?

<!--
Status: Complete Brazilian Portuguese translation prepared for author review
on August 21, 2026. It is preserved as the baseline for comparison with the
active draft in `src/content/post/a-era-10x-chegou.md`.
-->
...
fechados. Pull requests são integrados. Funcionalidades chegam à coluna de
concluídas. Construir software de repente parece barato. Precisamos chegar a
10x!

Parece um sonho realizado. O agente escreve uma funcionalidade, os testes
passam, os revisores aprovam e o código é integrado. A empresa recebe uma
funcionalidade pronta por menos do que teria gastado antes. É difícil não ficar
de queixo caído.

Mas há um porém. Escrevemos código e construímos software, mas isso descreve o
que fazemos, não por que fazemos. O valor não está no código. O investimento só
se paga quando alguém usa o sistema em execução para resolver um problema. É
nessa relação entre software rodando e um usuário satisfeito que mora o valor.
...
## A mágica econômica do software

Impérios foram construídos sobre software porque a mesma implementação pode
servir a milhares de pessoas durante anos. Um comportamento útil ganha escala
entre todas elas. Um defeito também. Essa escala funciona nos dois sentidos.
...
com menos código, esse é um resultado melhor, não uma realização menor.

Trabalho com software há mais de trinta anos, e o desejo de remover a restrição
dos desenvolvedores não é novo. Desenvolvedores são escassos e caros porque seu
trabalho e julgamento podem criar um ativo com esse tipo de alavancagem. O
mercado vive tentando obter o retorno escalável sem esperar pelo insumo escasso
nem pagar por ele.

Ferramentas de desenvolvimento rápido de aplicações, plataformas low-code e
plataformas no-code resolveram problemas localizados. Elas baratearam a
construção de certos tipos de software, mas não chegaram a eliminar o
desenvolvimento de software. Os desenvolvedores subiram um nível, para a
arquitetura, a integração, o design e o julgamento que as ferramentas não
conseguiam conter.

As transformações maiores mudaram mais do que a construção. A Internet mudou a
distribuição e, com ela, a forma de construir e atualizar software. O movimento
Lean Startup atacou o risco de investimento antes do product-market fit com
pequenos experimentos destinados a descobrir se havia algo valioso. Quando
havia, a empresa podia [se concentrar inteiramente no
crescimento](https://paulgraham.com/earn.html).

Agentes de programação podem ser a tecnologia mais poderosa das nossas vidas.
O perigo é usar esse poder para aumentar o estoque dentro do mesmo pipeline de
software. É a resposta que continuo vendo por toda a indústria: se a previsão
era de que um roadmap levaria dois anos, talvez os agentes consigam terminá-lo
neste trimestre. Mas um roadmap continua sendo uma lista de investimentos que
a empresa espera transformar em valor. Concluí-lo mais rápido não melhora essa
esperança. Pode fazer a empresa parecer mais rápida nas entregas enquanto seu
software se degrada mais rápido.

## Um dev e um agente entram num PR a 160 km/h

O pipeline de software que herdamos foi construído em torno de uma restrição
real. Construir software era lento, e construir a coisa errada colocava um
grande investimento em risco. O trabalho precisava ser dividido entre
desenvolvedores avançando em velocidades diferentes e depois reconciliado
repetidamente até que o sistema inteiro pudesse rodar.
...
reconstruíam a intenção e o risco a partir do diff. A CI verificava se o futuro
proposto poderia se reintegrar ao produto atual. Quando as verificações passavam
e as pessoas concordavam, a branch se tornava parte da linha do tempo chamada
estável.

Essa cerimônia não era arbitrária: coordenava trabalho humano escasso, limitava
o risco acumulado antes da produção e resistia à entropia. Um atalho provisório
pode ajudar uma startup a encontrar product-market fit e depois endurecer como
uma fundação que toda mudança posterior precisa acomodar.

Os agentes fazem as linhas do tempo alternativas avançarem mais rápido. Eles
não eliminam a colisão.

A maioria das empresas que observo mantém todas as fronteiras e multiplica o
que entra nelas. Gerentes de produto, designers, equipes de suporte e gestores
agora podem criar mudanças ao lado dos desenvolvedores. Agentes abrem mais pull
requests, e mais funcionalidades correm em direção ao lançamento. O primeiro
resultado é mais estoque esperando por integração e julgamento, não mais valor.

A [lei de Brandolini](https://en.wikipedia.org/wiki/Brandolini%27s_law) diz que
a energia necessária para refutar uma besteira é uma ordem de grandeza maior do
que a necessária para produzi-la. Mudanças geradas por agentes criam uma
assimetria parecida. Gerar uma mudança plausível é barato. Provar que ela merece
fazer parte de um sistema em execução não é. Uma pull request empurra essa
mudança adiante para que outra pessoa a reconstrua e aceite, geralmente um
desenvolvedor. Acoplamento oculto, consequências arquiteturais, invariantes
quebradas e comportamentos não testados ainda exigem julgamento. A geração se
expande. A responsabilidade se concentra.

Este é o problema central: o caminho até o valor está fragmentado pela empresa.
...
máquina que podia continuar produzindo enquanto ele dormia lhe deu mais um
motivo para continuar conferindo enquanto estava acordado.
...
A Toyota não obteve paralelismo seguro exigindo que os trabalhadores vigiassem
mais rápido. Ela redesenhou o maquinário até que uma pessoa pudesse cuidar de
várias máquinas.

É isso que a indústria entende ao contrário sobre agentes de programação:
adicionar mais agentes enquanto cada resultado ainda espera pela reconstrução
humana na próxima etapa não cria alavancagem. Só alimenta o mesmo
congestionamento na velocidade das máquinas.

## Um único alvo: Engenharia Profunda™

Uma tecnologia tão poderosa me dá motivos para repensar o desenvolvimento de
software a partir dos primeiros princípios, em vez de empurrar mais trabalho
para o maquinário antigo.

Vi um uso diferente dessa capacidade em um dos meus projetos. O trabalho exigia
uma estratégia que ninguém dentro da empresa havia usado. Sem agentes, eu teria
recorrido à experiência, a alguns experimentos que coubessem no orçamento e ao
melhor palpite disponível dentro do prazo.
...
A velocidade impressiona, mas não é isso que quero multiplicar. Exploração
barata me permitiu evitar um compromisso com a primeira resposta plausível. O
...
julgamento. Em vez de malabarizar seis funcionalidades sem relação entre si,
pude continuar aprofundando uma única visão.
